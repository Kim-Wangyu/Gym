...
public class ApplyController {
	...
	public ModelAndView addApply(ApplyDTO applyDTO, String[] addDay, String[] addTime, HttpSession session)
			throws Exception {
	...
	public ModelAndView apply(HttpSession session, ApplyDTO applyDTO, Long memberNum, Date day) throws Exception {
		...
		if (membershipDTO == null) {
			...
			applyDTO.setTraNum(trainerDTO.getTraNum());
			ar = applyService.findApplyNum(applyDTO);// day, time, trinum이 들어가야함
			if (ar != null) {
		...
		// 신청날짜가 현재날짜보다 이전일때
		if (trainerCheck && countCheck && applyDay && membershipCheck) {
